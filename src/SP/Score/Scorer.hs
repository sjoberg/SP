...
import Data.List.Stream  -- (foldl', partition)
...
import Prelude hiding (any,concatMap,filter,map,null,(++))

operatorScore :: Double -> (ObjectCluster,ObjectCluster) -> OperatorScore
operatorScore ta (oi,oj) = OperatorScore bestScore op (ocId oi,ocId oj)
  where
  value = if null magta then 0 else bestScore
...
maxOperatorScores top = get.argmaxes opScrVal.f.operatorScores
  where f = filter (\s -> opScrVal s > top)
        get []             = []
        get (score:scores) = score:get (filter (isIndependentOf score) scores)
        s1 `isIndependentOf` s2 = ((/=) `on` fst.mergeTuple) s1 s2 &&
                                  ((/=) `on` snd.mergeTuple) s1 s2
